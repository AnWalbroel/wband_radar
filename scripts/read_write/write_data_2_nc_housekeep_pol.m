...
if (data.DualPol > 0) && isfield(data, 'PNh')
    id_PNh = netcdf.defVar(ncid,'PNh','nc_float',[did_range,did_time]);
    netcdf.putAtt(ncid,id_PNh,'long_name','Total IF power in cross (horizontal) polarization measured at ADC input');
    netcdf.defVarFill(ncid,id_PNh,false,NaN('single'))
    netcdf.putAtt(ncid,id_PNh,'rpg_name','PNh');
end
...
if (data.DualPol > 0) && isfield(data, 'PNh')
    netcdf.defVarDeflate(ncid,id_PNh,true,true,5);
end
