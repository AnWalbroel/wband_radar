function fh = write_data_2_nc_moments_pol(data, outfile, config)
% function to write moments and most important metadata into netcdf4 file
% RG 1.6.2022
% added polarimetric variables - A. Walbroel 2026-02-16
...
if data.DualPol > 0
    ...
    netcdf.putAtt(ncid,id_vm_hv,'comment',['negative velocities indicate particles moving downwards'])
...
if data.DualPol > 0
    netcdf.defVarDeflate(ncid,id_ldr,true,true,5);
    netcdf.defVarDeflate(ncid,id_Ze_hv,true,true,5);
    netcdf.defVarDeflate(ncid,id_vm_hv,true,true,5);
